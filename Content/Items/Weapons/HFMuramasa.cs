        ...
        if (drawInfo.shadow == 0f)
        {
            ...
            if (timer > 0)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    0.2f + clampTimer * 0.005f + MathF.Sin((timer - clampTimer) * 0.04f) * 0.1f, // Scale slowly increases, then locks to max size, but oscillates using the sine of timer, clampTimer is subtracted to delay it
                    drawInfo.drawPlayer.direction == 1 ? SpriteEffects.FlipVertically : SpriteEffects.None,
                    0
                    ));
                    drawInfo.DrawDataCache.Add(new DrawData(
...
public class HFMuramasaProjectile : ModProjectile
{
    ...
    public string Glowmask => "GivlsWeapons/Content/Items/Weapons/HFMuramasaGlowmask";
