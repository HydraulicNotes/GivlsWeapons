...
namespace GivlsWeapons.Graphics
{
    public class VertexStrip
    {
        ...
        private int vertexesMaintained;
        ...
        public void PrepareStrip(Vector2[] positions, float[] rotations, StripColorFunction colorFunction, StripHalfWidthFunction widthFunction, Vector2 offsetForAllPositions = default(Vector2), int? expectedVertexPairsAmount = null, bool includeBacksides = false)
        {
            ...
            vertexesMaintained = numberOfPositions * 2;
            int num2 = vertexesMaintained;
            if (_vertices.Length < num2)
            {
                Array.Resize(ref _vertices, num2);
            }
            int num3 = numberOfPositions;
            ...
            for (int i = 0; i < numberOfPositions; i++)
            {
                ...
                {
                    numberOfPositions = i - 1;
                    vertexesMaintained = numberOfPositions * 2;
                    break;
                }
                Vector2 vertexPos = positions[i] + offsetForAllPositions;
                float vertexRot = MathHelper.WrapAngle(rotations[i]);
                int indexOnVertexArray = i * 2;
                float progressOnStrip = i / (float)(num3 - 1);
                AddVertex(colorFunction, widthFunction, vertexPos, vertexRot, indexOnVertexArray, progressOnStrip);
            }
            PrepareIndices(numberOfPositions, includeBacksides);
        }
        ...
        {
            if (vertexesMaintained >= 3)
            {
                Main.instance.GraphicsDevice.DrawUserIndexedPrimitives(PrimitiveType.TriangleList, _vertices, 0, vertexesMaintained, _indices, 0, _indicesAmountCurrentlyMaintained / 3);
            }
        }
    }
}